...
class ImitationTask(object):
    ...
    def _calc_reward_root_velocity(self):
        ...
        root_velocity_reward = np.exp(
            -self._root_velocity_err_scale * root_velocity_err
        )
    ...
    def _load_ref_motions(self, filenames):
        ...
        logging.info(
            "Loaded {:d} motion clips with {:.3f}s of motion data.".format(
        ...
        for j in range(num_joints):
            ...
            if j_pose_idx < 0:
                ...
                if j == 0:
                    ...
                    j_pose_idx = (
                        self._joint_pose_idx[j - 1] + self._joint_pose_size[j - 1]
                    )
    ...
    def _update_ref_motion(self):
        ...
        if (
            self._enable_cycle_sync and (new_phase < self._prev_motion_phase)
        ) or change_clip:
    ...
    def get_pose_size(self):
        ...
        pose_size = self._get_joint_pose_idx(
            num_joints - 1
        ) + self._get_joint_pose_size(num_joints - 1)
        return pose_size

    def get_vel_size(self):
        """Get the total size of a velocity array."""
        num_joints = self._get_num_joints()
        vel_size = self._get_joint_vel_idx(num_joints - 1) + self._get_joint_vel_size(
            num_joints - 1
        )
        return vel_size
    ...
    def _calc_ref_pose(self, time, apply_origin_offset=True):
        ...
        motion = self.get_active_motion()
        enable_warmup_pose = (
            self._curr_episode_warmup and time >= -self._warmup_time and time < 0.0
        )
    ...
    def _calc_ref_vel(self, time):
        ...
        root_ang_vel = pose3d.QuaternionRotatePoint(
            root_ang_vel, self._origin_offset_rot
        )
