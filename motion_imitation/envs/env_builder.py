...
from motion_imitation.envs.env_wrappers import (default_task, imitation_task,
                                                imitation_wrapper_env)
from motion_imitation.envs.env_wrappers import \
    observation_dictionary_to_array_wrapper
from motion_imitation.envs.env_wrappers import \
    observation_dictionary_to_array_wrapper as obs_dict_to_array_wrapper
from motion_imitation.envs.env_wrappers import (
    simple_forward_task, simple_openloop, trajectory_generator_wrapper_env)
from motion_imitation.envs.sensors import (environment_sensors, robot_sensors,
                                           sensor_wrappers)
...
def build_imitation_env(
    ...
    if enable_randomizer:
        randomizer = (
            controllable_env_randomizer_from_config.ControllableEnvRandomizerFromConfig(
                verbose=False
            )
        )
    ...
    env = trajectory_generator_wrapper_env.TrajectoryGeneratorWrapperEnv(
        env, trajectory_generator=trajectory_generator
    )
