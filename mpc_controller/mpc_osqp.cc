...
public:
  ...
  // -> X".
  std::vector<double>
  ComputeContactForces(std::vector<double> com_position,
...
ConvexMpc::ConvexMpc(double mass, const std::vector<double> &inertia,
    ...
    : mass_(mass), inv_mass_(1 / mass), inertia_(inertia.data()),
      ...
      action_dim_(num_legs * k3Dim), state_(kStateDim),
      ...
      a_mat_(kStateDim, kStateDim), b_mat_(kStateDim, action_dim_),
      ab_concatenated_(kStateDim + action_dim_, kStateDim + action_dim_),
      a_exp_(kStateDim, kStateDim), b_exp_(kStateDim, action_dim_),
